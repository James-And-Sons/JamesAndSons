/**
 * Zoho Inventory & Books Sales Order Sync
 */
...
async function getZohoAccessToken(): Promise<string> {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Failed to refresh Zoho token: ${errorData.error || res.statusText}`,
    );
  }
...
export async function syncOrderToZoho(orderId: string): Promise<string> {
  console.log(
    `[Zoho Integration] Starting order sync for order ID: ${orderId}`,
  );
  ...
  const orgId =
    ...
    process.env.NEXT_PUBLIC_ZOHO_DESK_ORG_ID ||
    "";
  ...
  const salesOrderPayload = {
    ...
    shipment_date: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)
      .toISOString()
      .split("T")[0],
    ...
    line_items: order.items.map((item) => ({
      ...
      description: item.warrantyPlanSku
        ? `Warranty: ${item.warrantyPlanName}`
        : undefined,
    })),
  ...
  console.log(
    `[Zoho Integration] Successfully created Zoho Sales Order: ${resData.salesorder.salesorder_id}`,
  );
