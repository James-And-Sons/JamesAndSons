/**
 * Amazon Selling Partner API (SP-API) core client utilities.
 * Uses native Node.js crypto for AWS SigV4 signing.
 */
...
export async function getRestrictedDataToken(
  ...
  const key = dataElements.slice().sort().join(",");
  if (
    _cachedRdt &&
    _cachedRdt.dataElements === key &&
    Date.now() < _cachedRdt.expiresAt - 120_000
  ) {
    return _cachedRdt.token;
  }
  ...
  if (!res.ok) {
    const text = await res.text();
    console.warn(
      `[Amazon SP-API] RDT request returned ${res.status}: ${text}. Falling back to standard LWA access token.`,
    );
    return accessToken;
  }

  const data = await res.json();
  const rdt: string = data.restrictedDataToken;
