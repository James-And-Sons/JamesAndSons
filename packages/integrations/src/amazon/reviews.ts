...
export async function dispatchAmazonReviewSolicitations(): Promise<SolicitationsResult> {
  ...
  const stats: SolicitationsResult = {
    processed: 0,
    success: 0,
    skipped: 0,
    errors: 0,
  ...
  try {
    ...
    console.error(
      "[Amazon Solicitations] Failed to load credentials or access token:",
      err,
    );
    stats.errors++;
    return stats;
  ...
  console.log(
    `[Amazon Solicitations] Querying Amazon orders created between ${thirtyDaysAgo.toISOString()} and ${fiveDaysAgo.toISOString()}...`,
  );
  ...
  console.log(
    `[Amazon Solicitations] Found ${orders.length} candidate orders for review request.`,
  );
  ...
  for (const order of orders) {
    ...
    try {
      ...
      console.log(
        `[Amazon Solicitations] Soliciting feedback for Amazon Order: ${amazonOrderId}...`,
      );
      ...
      if (res.ok) {
        console.log(
          `[Amazon Solicitations] ✅ Review requested successfully for order ${amazonOrderId}`,
        );
        stats.success++;
        ...
        console.warn(
          `[Amazon Solicitations] API returned status ${res.status} for order ${amazonOrderId}: ${bodyText}`,
        );

        if (res.status === 400 || res.status === 403 || res.status === 412) {
          console.log(
            `[Amazon Solicitations] Order ${amazonOrderId} is ineligible or request already dispatched. Skipping permanently.`,
          );
          stats.skipped++;
      ...
      console.error(
        `[Amazon Solicitations] Error soliciting order ${amazonOrderId}:`,
        err,
      );
      stats.errors++;
    }
  ...
  console.log(
    `[Amazon Solicitations] Run complete. Processed: ${stats.processed}, Success: ${stats.success}, Skipped: ${stats.skipped}, Errors: ${stats.errors}`,
  );
  return stats;
}
