...
export default function ProductFormClient({
  ...
  const [editingImageTarget, setEditingImageTarget] = useState<{
    type: "images" | "whiteBg" | "variantImages" | "variantWhiteBg";
    index: number;
    variantIndex?: number;
  } | null>(null);

  const handleSaveEditedImageInForm = async (blob: Blob) => {
    ...
    try {
      ...
      const { type, index, variantIndex } = editingImageTarget;
      if (type === "images") {
        setImages((prev) => prev.map((img, i) => (i === index ? newUrl : img)));
      } else if (type === "whiteBg") {
        setWhiteBackgroundImages((prev) =>
          prev.map((img, i) => (i === index ? newUrl : img)),
        );
      } else if (type === "variantImages" && variantIndex !== undefined) {
        const currentList = variants[variantIndex].images
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean);
        const updated = currentList.map((img, i) =>
          i === index ? newUrl : img,
        );
        updateVariantField(variantIndex, "images", updated.join(", "));
      } else if (type === "variantWhiteBg" && variantIndex !== undefined) {
        const currentList = (variants[variantIndex].whiteBackgroundImages || "")
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean);
        const updated = currentList.map((img, i) =>
      ...
      console.error("Error saving edited image in form:", err);
  ...
  const [parentValues, setParentValues] = useState({
    ...
    actualHeight: defaultValues?.actualHeight
      ? String(defaultValues.actualHeight)
      : "",
    actualWidth: defaultValues?.actualWidth
      ? String(defaultValues.actualWidth)
      : "",
    actualDepth: defaultValues?.actualDepth
      ? String(defaultValues.actualDepth)
      : "",
    ...
    googleProductCategory:
      defaultValues?.googleProductCategory ||
      getLogicalDefaultsForCategory(
        categories.find((c) => c.id === defaultValues?.categoryId)?.name || "",
      ).googleProductCategory,
    amazonFixtureForm:
      defaultValues?.amazonFixtureForm ||
      getLogicalDefaultsForCategory(
        categories.find((c) => c.id === defaultValues?.categoryId)?.name || "",
      ).amazonFixtureForm,
    amazonMountingType:
      defaultValues?.amazonMountingType ||
      getLogicalDefaultsForCategory(
        categories.find((c) => c.id === defaultValues?.categoryId)?.name || "",
      ).amazonMountingType,
    amazonLightingMethod:
      defaultValues?.amazonLightingMethod ||
      getLogicalDefaultsForCategory(
        categories.find((c) => c.id === defaultValues?.categoryId)?.name || "",
      ).amazonLightingMethod,
    amazonWaterResistance:
      defaultValues?.amazonWaterResistance ||
      getLogicalDefaultsForCategory(
        categories.find((c) => c.id === defaultValues?.categoryId)?.name || "",
      ).amazonWaterResistance,
    amazonTheme:
      defaultValues?.amazonTheme ||
      getLogicalDefaultsForCategory(
        categories.find((c) => c.id === defaultValues?.categoryId)?.name || "",
      ).amazonTheme,
    amazonSpecialFeatures:
      defaultValues?.amazonSpecialFeatures || ([] as string[]),
  ...
  const handleApplyAiListing = () => {
    ...
    setParentValues((prev) => ({
      ...
      bulletPoints: Array.isArray(aiResult.bulletPoints)
        ? aiResult.bulletPoints.join("\n")
        : aiResult.bulletPoints || prev.bulletPoints,
      materialAndFinish: Array.isArray(aiResult.materialAndFinish)
        ? aiResult.materialAndFinish.join(", ")
        : aiResult.materialAndFinish || prev.materialAndFinish,
      bulbType: Array.isArray(aiResult.bulbType)
        ? aiResult.bulbType.join(", ")
        : aiResult.bulbType || prev.bulbType,
      style: Array.isArray(aiResult.style)
        ? aiResult.style.join(", ")
        : aiResult.style || prev.style,
      ...
      gstRate:
        typeof aiResult.gstRate === "number" ? aiResult.gstRate : prev.gstRate,
      ...
      googleProductCategory:
        aiResult.googleProductCategory || prev.googleProductCategory,
      amazonFixtureForm: aiResult.amazonFixtureForm || prev.amazonFixtureForm,
      amazonMountingType:
        aiResult.amazonMountingType || prev.amazonMountingType,
      amazonLightingMethod:
        aiResult.amazonLightingMethod || prev.amazonLightingMethod,
      amazonWaterResistance:
        aiResult.amazonWaterResistance || prev.amazonWaterResistance,
      amazonTheme: aiResult.amazonTheme || prev.amazonTheme,
      amazonSpecialFeatures: Array.isArray(aiResult.amazonSpecialFeatures)
        ? aiResult.amazonSpecialFeatures
        : aiResult.amazonSpecialFeatures || prev.amazonSpecialFeatures,
      amazonIncludedComponents:
        aiResult.amazonIncludedComponents || prev.amazonIncludedComponents,
  ...
  const showB2BWarning = !!(
    parentValues.b2bPrice &&
    parentValues.mrp &&
    Number(parentValues.b2bPrice) === Number(parentValues.mrp)
  );
  ...
  const isBasicComplete = !!(
    parentValues.name &&
    parentValues.sku &&
    parentValues.categoryId
  );
  const isPricingComplete = !!(
    parentValues.mrp &&
    parentValues.d2cPrice &&
    parentValues.b2bPrice &&
    parentValues.stockQuantity
  );
  ...
  const isVarPricingComplete =
    activeTab !== "parent" && typeof activeTab === "number";
  const isVarDimensionsComplete =
    activeTab !== "parent" && typeof activeTab === "number";
  const isVarSpecsComplete =
    activeTab !== "parent" && typeof activeTab === "number";
  const isVarPlatformComplete =
    activeTab !== "parent" && typeof activeTab === "number";
  const isVarImagesComplete =
    activeTab !== "parent" &&
    typeof activeTab === "number" &&
    !!(
  ...
  const handleParentFieldChange = (
    field: keyof typeof parentValues,
    val: any,
  ) => {
    setIsDirty(true);
    setParentValues((prev) => {
        ...
        if (cat) {
          ...
          updated.gstRate =
            cat.gstRate !== null && cat.gstRate !== undefined
              ? cat.gstRate
              : 18;
          updated.bisCertification =
            cat.bisStandard || updated.bisCertification || "";

          const inferred = getLogicalDefaultsForCategory(cat.name);
          if (
            !updated.amazonFixtureForm ||
            updated.amazonFixtureForm === "Light Fixture"
          )
            updated.amazonFixtureForm = inferred.amazonFixtureForm;
          if (
            !updated.amazonMountingType ||
            updated.amazonMountingType === "Ceiling / Wall Mount"
          )
            updated.amazonMountingType = inferred.amazonMountingType;
          if (!updated.amazonLightingMethod)
            updated.amazonLightingMethod = inferred.amazonLightingMethod;
          if (!updated.amazonWaterResistance)
            updated.amazonWaterResistance = inferred.amazonWaterResistance;
          if (!updated.amazonTheme) updated.amazonTheme = inferred.amazonTheme;
          if (!updated.googleProductCategory)
            updated.googleProductCategory = inferred.googleProductCategory;
        }
      }
      return updated;
    });
  ...
  const addVariant = () => {
    ...
    const autoSku = parentValues.sku
      ? `${parentValues.sku}${defaultSkuSuffix}`.toUpperCase()
      : "";
  ...
  const handleImageDrop = (
    ...
    if (variantIndex !== undefined) {
      // VARIANT IMAGE DROP
      const srcField =
        sourceType === "whiteBg" ? "whiteBackgroundImages" : "images";
      const tgtField =
        targetType === "whiteBg" ? "whiteBackgroundImages" : "images";

      let srcList = (variants[variantIndex][srcField] || "")
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
      let tgtList =
        sourceType === targetType
          ? srcList
          : (variants[variantIndex][tgtField] || "")
              .split(",")
              .map((s) => s.trim())
              .filter(Boolean);
      ...
      if (sourceType === targetType) {
        const insertAt =
          targetIndex !== undefined ? targetIndex : srcList.length;
        srcList.splice(insertAt, 0, movedUrl);
        updateVariantField(variantIndex, srcField, srcList.join(", "));
      } else {
        const insertAt =
          targetIndex !== undefined ? targetIndex : tgtList.length;
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      actualHeight: parentValues.actualHeight
        ? parseFloat(parentValues.actualHeight)
        : null,
      actualWidth: parentValues.actualWidth
        ? parseFloat(parentValues.actualWidth)
        : null,
      actualDepth: parentValues.actualDepth
        ? parseFloat(parentValues.actualDepth)
        : null,
      materialAndFinish: parentValues.materialAndFinish
        ? parentValues.materialAndFinish
            .split(",")
            .map((s: string) => s.trim())
            .filter(Boolean)
        : [],
      bulbType: parentValues.bulbType
        ? parentValues.bulbType
            .split(",")
            .map((s: string) => s.trim())
            .filter(Boolean)
        : [],
      style: parentValues.style
        ? parentValues.style
            .split(",")
            .map((s: string) => s.trim())
            .filter(Boolean)
      ...
      bulletPoints: parentValues.bulletPoints
        ? parentValues.bulletPoints
            .split("\n")
            .map((s: string) => s.trim())
            .filter(Boolean)
        : [],
      images: images.filter((img: string) => img.trim()),
      whiteBackgroundImages: whiteBackgroundImages.filter((img: string) =>
        img.trim(),
      ),
      dimensionUnit,
      ...
      variants: variants.map((v) => ({
        ...
        style: v.style
          ? v.style
              ...
              .filter(Boolean)
          : [],
        specs: v.specs.reduce((acc: any, s: Spec) => {
          if (s.key) acc[s.key] = s.value;
          return acc;
        }, {}),
      })),
      specs: specs.reduce((acc: any, s: Spec) => {
        if (s.key) acc[s.key] = s.value;
        return acc;
      }, {}),
    };
    ...
    try {
      const url =
        mode === "add" ? "/api/products" : `/api/products/${defaultValues.id}`;
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
      {error && (
        <div className="p-4 border border-red-900 bg-red-900/10 text-red-400 font-mono text-[12px]">
          {error}
        </div>
      )}
      ...
      <div className="lg:hidden sticky top-0 z-30 bg-background/95 backdrop-blur border-b border-border py-4 px-6 -mx-6 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-6">
          ...
          <div
            className={`flex items-center gap-2 font-mono text-[10px] uppercase tracking-wider ${isDirty ? "text-amber-500" : "text-emerald-500"}`}
          >
            <span
              className={`w-2 h-2 rounded-full ${isDirty ? "bg-amber-500 animate-pulse" : "bg-emerald-500"}`}
            ></span>
        ...
        {activeTab === "parent" ? (
          <div className="space-y-6">
            ...
            <div className="premium-card p-4 rounded-lg bg-surface border border-accent/30 flex flex-col gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="font-mono text-[9px] text-muted uppercase tracking-wider">
                      Auto-fill all specs, listing narrative, and marketplace
                      SEO attributes
                    </div>
                  </div>
                </div>
              ...
              {showAiAssistant && (
                <div className="pt-3 border-t border-border/40 space-y-4 font-sans">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <CustomDropdown
                        ...
                        options={[
                          {
                            value: "Luxurious & Regal",
                            label: "Luxurious & Regal",
                          },
                          {
                  ...
                  <div>
                    <label className="font-mono text-[9px] uppercase tracking-widest text-muted block mb-1">
                      Keywords / Materials / Design Highlights
                    </label>
                    <textarea
                      value={aiKeywords}
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    ...
                    >
                      {generatingAi
                        ? "Generating Listing..."
                        : "✨ Generate Full Listing Copy & SEO"}
                    </button>
                  </div>
                    ...
                    <div className="mt-4 p-4 bg-background/80 border border-accent/40 rounded-sm space-y-3">
                      <div className="font-mono text-[9px] uppercase tracking-widest text-accent font-semibold flex justify-between items-center">
                        ...
                        <span className="text-muted font-normal">
                          All fields ready to auto-populate
                        </span>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <div className="font-sans text-[12px] text-secondary whitespace-pre-wrap leading-relaxed">
                      ...
                      <div className="pt-2 border-t border-border/30">
                        <div className="font-mono text-[9px] text-accent uppercase font-semibold mb-1.5">
                          Generated Specifications &amp; Marketplace Attributes
                        </div>
                        <div className="flex flex-wrap gap-1.5 font-mono text-[9px]">
                          {aiResult.material && (
                            <span className="bg-surface border border-border px-2 py-0.5 rounded text-secondary">
                              Material: {aiResult.material}
                            </span>
                          )}
                          {aiResult.power && (
                            <span className="bg-surface border border-border px-2 py-0.5 rounded text-secondary">
                              Power: {aiResult.power}
                            </span>
                          )}
                          {aiResult.voltage && (
                            <span className="bg-surface border border-border px-2 py-0.5 rounded text-secondary">
                              Voltage: {aiResult.voltage}
                            </span>
                          )}
                          {aiResult.hsnCode && (
                            <span className="bg-surface border border-border px-2 py-0.5 rounded text-secondary">
                              HSN: {aiResult.hsnCode}
                            </span>
                          )}
                          {aiResult.amazonFixtureForm && (
                            <span className="bg-surface border border-border px-2 py-0.5 rounded text-secondary">
                              Form: {aiResult.amazonFixtureForm}
                            </span>
                          )}
                          {aiResult.amazonMountingType && (
                            <span className="bg-surface border border-border px-2 py-0.5 rounded text-secondary">
                              Mount: {aiResult.amazonMountingType}
                            </span>
                          )}
                          {aiResult.amazonKeywords && (
                            <span className="bg-surface border border-border px-2 py-0.5 rounded text-secondary truncate max-w-full">
                              Keywords: {aiResult.amazonKeywords}
                            </span>
                          )}
                        </div>
                      </div>
            ...
            <CollapsibleCard
              ...
              onToggle={() =>
                setOpenSections((prev) => ({ ...prev, basic: !prev.basic }))
              }
            >
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleParentFieldChange("name", e.target.value)
                    }
                    className={inputCls}
                ...
                <div>
                  <label htmlFor="skuCode" className={labelCls}>
                    SKU Code *
                  </label>
                  <input
                    id="skuCode"
                    ...
                    onChange={(e) =>
                      handleParentFieldChange("sku", e.target.value)
                    }
                ...
                <div className="col-span-2">
                  <div className="flex justify-between items-baseline mb-1">
                    ...
                    <span className="font-mono text-[9px] text-muted">
                      {(parentValues.description || "").length} / 2000
                    </span>
                  </div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      handleParentFieldChange("description", e.target.value)
                    }
                    rows={4}
                    className={inputCls}
                ...
                <div>
                  ...
                  <CustomDropdown
                    ...
                    onChange={(val) =>
                      handleParentFieldChange("categoryId", val)
                    }
            ...
            <CollapsibleCard
              ...
              onToggle={() =>
                setOpenSections((prev) => ({ ...prev, pricing: !prev.pricing }))
              }
            >
              <div className="grid grid-cols-4 gap-6">
                <div>
                  ...
                  <div className="flex items-center">
                    <span className="bg-surface-muted border border-r-0 border-border px-3 py-3 font-mono text-[13px] text-muted">
                      ₹
                    </span>
                    <input
                      id="mrp"
                      ...
                      onChange={(e) =>
                        handleParentFieldChange("mrp", e.target.value)
                      }
                      className={inputCls}
                    />
                  </div>
                ...
                <div>
                  <label htmlFor="d2cPrice" className={labelCls}>
                    D2C Price (₹) *
                  </label>
                  <div className="flex items-center">
                    <span className="bg-surface-muted border border-r-0 border-border px-3 py-3 font-mono text-[13px] text-muted">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleParentFieldChange("d2cPrice", e.target.value)
                      }
                      className={inputCls}
                    />
                  </div>
                ...
                <div>
                  ...
                  <div className="flex items-center">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleParentFieldChange("b2bPrice", e.target.value)
                      }
                      className={inputCls}
                    />
                  </div>
                  {showB2BWarning && (
                    <p className="text-[10px] text-amber-400 font-mono mt-1">
                      ⚠ Equal to MRP — check if intended
                    </p>
                  )}
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleParentFieldChange("stockQuantity", e.target.value)
                    }
                    className={inputCls}
                  />
                </div>
                ...
                <div className="grid grid-cols-4 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleParentFieldChange("weight", e.target.value)
                      }
                      className={inputCls}
                      placeholder="0.5"
                    />
                  ...
                  <div>
                    <label htmlFor="length" className={labelCls}>
                      Length (cm)
                    </label>
                    <input
                      id="length"
                      ...
                      onChange={(e) =>
                        handleParentFieldChange("length", e.target.value)
                      }
                      className={inputCls}
                      placeholder="10"
                    />
                  ...
                  <div>
                    <label htmlFor="breadth" className={labelCls}>
                      Breadth (cm)
                    </label>
                    <input
                      id="breadth"
                      ...
                      onChange={(e) =>
                        handleParentFieldChange("breadth", e.target.value)
                      }
                      className={inputCls}
                      placeholder="10"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleParentFieldChange("height", e.target.value)
                      }
                      className={inputCls}
                      placeholder="10"
                    />
                ...
                <div className="grid grid-cols-3 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleParentFieldChange("actualHeight", e.target.value)
                      }
                      className={inputCls}
                      placeholder="—"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleParentFieldChange("actualWidth", e.target.value)
                      }
                      className={inputCls}
                      placeholder="—"
                    />
                  ...
                  <div>
                    <label className={labelCls}>
                      Depth ({dimensionUnit === "CM" ? "cm" : "in"}) (Optional)
                    </label>
                    <input
                      type="number"
                      step="0.1"
                      ...
                      onChange={(e) =>
                        handleParentFieldChange("actualDepth", e.target.value)
                      }
                      className={inputCls}
                      placeholder="—"
                    />
            ...
            <CollapsibleCard
              ...
              onToggle={() =>
                setOpenSections((prev) => ({ ...prev, specs: !prev.specs }))
              }
            >
              <div className="grid grid-cols-3 gap-6">
                <div>
                  ...
                  <input
                    value={
                      parentValues.gstRate ? `${parentValues.gstRate}%` : "—"
                    }
                    readOnly
              ...
              <div className="grid grid-cols-3 gap-6 pt-4 border-t border-border/40">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleParentFieldChange("bulbType", e.target.value)
                    }
                    className={inputCls}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleParentFieldChange("style", e.target.value)
                    }
                    className={inputCls}
              ...
              <div className="grid grid-cols-2 gap-6 pt-4 border-t border-border/40">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleParentFieldChange("power", e.target.value)
                    }
                    className={inputCls}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleParentFieldChange("voltage", e.target.value)
                    }
                    className={inputCls}
              ...
              <div className="pt-4 border-t border-border/40 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <button
                    type="button"
                    onClick={addSpec}
                    className="font-mono text-[10px] uppercase tracking-[0.12em] text-accent border border-accent/20 px-3 py-1.5 hover:border-accent hover:bg-accent/5 transition-all rounded-sm"
                  >
                    + Add Spec
                  </button>
                </div>
                {specs.map((spec) => (
                  <div key={spec.id} className="flex gap-4 items-center w-full">
                    ...
                    <button
                      ...
                      className="text-red-400 p-2 hover:bg-red-950/20 transition-colors rounded-sm"
                      title="Remove spec"
                    >
                      ×
                    </button>
            ...
            <CollapsibleCard
              ...
              onToggle={() =>
                setOpenSections((prev) => ({ ...prev, seo: !prev.seo }))
              }
            >
              <div className="grid grid-cols-3 gap-6">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleParentFieldChange("brand", e.target.value)
                    }
                    className={inputCls}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleParentFieldChange("warranty", e.target.value)
                    }
                    className={inputCls}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleParentFieldChange("countryOfOrigin", e.target.value)
                    }
                    className={inputCls}
                  />
                </div>
              ...
              <div className="border-t border-border/40 pt-4">
                <div className="font-mono text-[9px] uppercase tracking-widest text-muted mb-3">
                  Auto-detected on category or AI generation — edit to override
                </div>
                <div className="grid grid-cols-2 gap-6">
                  <div className="relative">
                    ...
                    {parentValues.amazonFixtureForm ? (
                      <span className="absolute right-3 top-[32px] font-mono text-[8px] uppercase tracking-widest bg-emerald-950/40 text-emerald-400 px-2 py-0.5 border border-emerald-500/30 rounded-sm">
                        auto · set
                      </span>
                    ) : (
                      <span className="absolute right-3 top-[32px] font-mono text-[8px] uppercase tracking-widest bg-surface-muted text-muted px-2 py-0.5 border border-border rounded-sm">
                        auto · pending
                      </span>
                    )}
                  </div>
              ...
              <div className="border-t border-border/40 pt-4">
                <div className="flex justify-between items-baseline mb-1">
                  <label htmlFor="amazonKeywords" className={labelCls}>
                    Generic search keywords
                  </label>
                  <span className="font-mono text-[9px] text-muted">
                    {(parentValues.amazonKeywords || "").length} / 250
                  </span>
                </div>
                <input
                  id="amazonKeywords"
                  ...
                  onChange={(e) =>
                    handleParentFieldChange("amazonKeywords", e.target.value)
                  }
              ...
              <div className="border-t border-border/40 pt-4">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {[
                    ...
                    const isChecked =
                      parentValues.amazonSpecialFeatures?.includes(feature);
                    return (
                      <button
                        key={feature}
                        type="button"
                        onClick={() => {
                          ...
                          const newFeatures = isChecked
                            ? (parentValues.amazonSpecialFeatures || []).filter(
                                (f: string) => f !== feature,
                              )
                            : [
                                ...(parentValues.amazonSpecialFeatures || []),
                                feature,
                              ];
                          handleParentFieldChange(
                            "amazonSpecialFeatures",
            ...
            <CollapsibleCard
              ...
              onToggle={() =>
                setOpenSections((prev) => ({ ...prev, images: !prev.images }))
              }
            >
              {/* 1. STOREFRONT IMAGES */}
              <div className="space-y-4">
                <div className="flex justify-between items-start">
                  <div>
                    <h4 className="font-serif text-[16px] text-accent tracking-wide">
                      Remastered Images (Storefront &amp; Socials)
                    </h4>
                    <p className="font-mono text-[10px] text-muted">
                      The first image is the cover photo shown on storefront
                      catalogs. Drag images to rearrange or move to White
                      Background section.
                    </p>
                  </div>
                ...
                <div
                  onDragOver={(e) => {
                    e.preventDefault();
                    if (dragOverZone !== "parent-remastered")
                      setDragOverZone("parent-remastered");
                  }}
                  onDragLeave={(e) => {
                    e.preventDefault();
                    if (dragOverZone === "parent-remastered")
                ...
                >
                  {images.length > 0 ? (
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                      {images.map((url, idx) => {
                        ...
                        const isBeingDragged =
                          draggedImage?.sourceType === "remastered" &&
                          draggedImage?.sourceIndex === idx &&
                          draggedImage?.variantIndex === undefined;
                        ...
                        return (
                          ...
                          >
                            ...
                            {idx === 0 && (
                              <span className="absolute top-2 left-2 bg-accent text-black font-semibold px-2 py-0.5 rounded font-mono text-[8px] uppercase tracking-wider">
                                Primary
                              </span>
                            )}
                            <button
                            ...
                            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center gap-2 transition-all">
                              ...
                              <button
                                ...
                                className="px-2 py-1 rounded bg-amber-400 text-black font-mono text-[10px] uppercase font-bold tracking-wider hover:bg-amber-300 cursor-pointer shadow"
                                title="Edit Image"
                              >
                                ✎ Edit
                              </button>
                    ...
                    <div className="text-center font-mono text-[11px] text-muted py-4">
                      Drag remastered images here or upload using the button
                      below.
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-4 pt-6 border-t border-border/40 mt-6">
                <div className="flex justify-between items-start">
                  <div>
                    <h4 className="font-serif text-[16px] text-accent tracking-wide">
                      White Background Images (Amazon Listing Compliance)
                    </h4>
                    <p className="font-mono text-[10px] text-muted">
                      Amazon requires the first listing photo to be on a pure
                      white background. Drag images between sections freely.
                    </p>
                  </div>
                  <span className="font-mono text-[9px] text-accent/80 bg-accent/10 border border-accent/20 px-2.5 py-1 rounded-full uppercase tracking-wider hidden sm:inline-block">
                    ⇄ Drag &amp; Drop Enabled
                ...
                <div
                  onDragOver={(e) => {
                    e.preventDefault();
                    if (dragOverZone !== "parent-whiteBg")
                      setDragOverZone("parent-whiteBg");
                  }}
                  onDragLeave={(e) => {
                    e.preventDefault();
                    if (dragOverZone === "parent-whiteBg")
                  ...
                  {whiteBackgroundImages.length > 0 ? (
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                      {whiteBackgroundImages.map((url, idx) => {
                        ...
                        const isBeingDragged =
                          draggedImage?.sourceType === "whiteBg" &&
                          draggedImage?.sourceIndex === idx &&
                          draggedImage?.variantIndex === undefined;
                        const isOver = dragOverCardKey === cardKey;
                    ...
                    <div className="text-center font-mono text-[11px] text-muted py-4">
                      Drag white background images here or upload using the
                      button below.
                    </div>
                  )}
                </div>
          ...
          <div className="space-y-6 animate-in fade-in duration-200">
            ...
            <div className="premium-card p-6 flex justify-between items-center border border-border bg-surface/10 rounded">
              <div>
                <h3 className="font-serif text-[20px] text-primary font-medium tracking-wide">
                  Variant overrides:{" "}
                  {variants[activeTab].name || `Variant ${activeTab + 1}`}
                </h3>
                <p className="font-mono text-[9px] text-muted uppercase tracking-widest mt-1">
                  Specify override values for this specific variant option
                </p>
              </div>
              <button
                type="button"
            ...
            <CollapsibleCard
              ...
              onToggle={() =>
                setOpenSections((prev) => ({ ...prev, v_basic: !prev.v_basic }))
              }
            >
              <div className="grid grid-cols-3 gap-6">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateVariantField(activeTab, "name", e.target.value)
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateVariantField(activeTab, "sku", e.target.value)
                    }
              ...
              <div className="grid grid-cols-3 gap-6">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateVariantField(activeTab, "mrp", e.target.value)
                    }
                    placeholder={
                      parentValues.mrp
                        ? `Inherit (₹${parentValues.mrp})`
                        : "Inherit"
                    }
                    className={inputCls}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateVariantField(activeTab, "d2cPrice", e.target.value)
                    }
                    placeholder={
                      parentValues.d2cPrice
                        ? `Inherit (₹${parentValues.d2cPrice})`
                        : "Inherit"
                    }
                    className={inputCls}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateVariantField(activeTab, "b2bPrice", e.target.value)
                    }
                    placeholder={
                      parentValues.b2bPrice
                        ? `Inherit (₹${parentValues.b2bPrice})`
                        : "Inherit"
                    }
                    className={inputCls}
                  />
            ...
            <CollapsibleCard
              ...
              onToggle={() =>
                setOpenSections((prev) => ({
                  ...prev,
                  v_dimensions: !prev.v_dimensions,
                }))
              }
            >
              <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-4 gap-6">
                    <div>
                      ...
                      <input
                        ...
                        placeholder={
                          parentValues.weight
                            ? `Inherit (${parentValues.weight} kg)`
                            : "Inherit"
                        }
                        className={inputCls}
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder={
                          parentValues.length
                            ? `Inherit (${parentValues.length} cm)`
                            : "Inherit"
                        }
                        className={inputCls}
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder={
                          parentValues.breadth
                            ? `Inherit (${parentValues.breadth} cm)`
                            : "Inherit"
                        }
                        className={inputCls}
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder={
                          parentValues.height
                            ? `Inherit (${parentValues.height} cm)`
                            : "Inherit"
                        }
                        className={inputCls}
                      />
                ...
                <div className="border-t border-border/40 pt-6">
                  <div className="font-mono text-[9px] uppercase tracking-widest text-accent mb-3">
                    Display Dimensions ({dimensionUnit === "CM" ? "cm" : "in"})
                    overrides
                  </div>
                  <div className="grid grid-cols-3 gap-6">
                    <div>
                      ...
                      <input
                        ...
                        placeholder={
                          parentValues.actualHeight
                            ? `Inherit (${parentValues.actualHeight})`
                            : "Inherit"
                        }
                        className={inputCls}
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder={
                          parentValues.actualWidth
                            ? `Inherit (${parentValues.actualWidth})`
                            : "Inherit"
                        }
                        className={inputCls}
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder={
                          parentValues.actualDepth
                            ? `Inherit (${parentValues.actualDepth})`
                            : "Inherit"
                        }
                        className={inputCls}
                      />
            ...
            <CollapsibleCard
              ...
              onToggle={() =>
                setOpenSections((prev) => ({ ...prev, v_specs: !prev.v_specs }))
              }
            >
              <div className="space-y-6">
                <div className="grid grid-cols-3 gap-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateVariantField(activeTab, "style", e.target.value)
                      }
                ...
                <div className="grid grid-cols-2 gap-6 pt-4 border-t border-border/40">
                  <div>
                    <label className={labelCls}>
                      Power specification override
                    </label>
                    <input
                      value={variants[activeTab].power}
                      onChange={(e) =>
                        updateVariantField(activeTab, "power", e.target.value)
                      }
                      placeholder={
                        parentValues.power
                          ? `Inherit (${parentValues.power})`
                          : "Inherit"
                      }
                      className={inputCls}
                    />
                  ...
                  <div>
                    <label className={labelCls}>
                      Voltage specification override
                    </label>
                    <input
                      value={variants[activeTab].voltage}
                      onChange={(e) =>
                        updateVariantField(activeTab, "voltage", e.target.value)
                      }
                      placeholder={
                        parentValues.voltage
                          ? `Inherit (${parentValues.voltage})`
                          : "Inherit"
                      }
                      className={inputCls}
                    />
                ...
                <div className="pt-6 border-t border-border/40 space-y-4">
                  <div className="flex justify-between items-center">
                    <label className={labelCls}>
                      Variant-specific specs overrides
                    </label>
                    <button
                      type="button"
                      onClick={() => addVariantSpec(activeTab)}
                      className="font-mono text-[10px] uppercase tracking-[0.12em] text-accent border border-accent/20 px-3 py-1.5 hover:border-accent hover:bg-accent/5 transition-all rounded-sm"
                    >
                      + Add Variant Spec
                    </button>
                  ...
                  {variants[activeTab].specs.map((spec) => (
                    ...
                    >
                      <input
                        ...
                        placeholder="Key (e.g. Frame Color)"
                        className={`${inputCls} flex-1 !py-2.5`}
                      />
                      <input
                        value={spec.value}
                        ...
                        placeholder="Value (e.g. Polished Chrome)"
                        className={`${inputCls} flex-1 !py-2.5`}
                      />
                      <button
                        type="button"
                        onClick={() => removeVariantSpec(activeTab, spec.id)}
                        className="text-red-400 p-2 hover:bg-red-950/20 transition-colors rounded-sm"
                        title="Remove spec"
                      >
                        ×
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateVariantField(activeTab, "brand", e.target.value)
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateVariantField(activeTab, "warranty", e.target.value)
                    }
                ...
                <div className="col-span-2">
                  <label className={labelCls}>
                    Google Product Category Override
                  </label>
                  <CustomDropdown
                    value={variants[activeTab].googleProductCategory || ""}
                    options={GOOGLE_PRODUCT_CATEGORIES.map((cat) => ({
                      value: cat,
                      label: cat,
                    }))}
                    onChange={(val) =>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="font-serif text-[16px] text-accent tracking-wide">
                        Variant Remastered Images
                      </h4>
                      <p className="font-mono text-[10px] text-muted">
                        Variant-specific marketing images (inherits parent if
                        left blank). Drag to reorder or move between sections.
                      </p>
                    </div>
                    <span className="font-mono text-[9px] text-accent/80 bg-accent/10 border border-accent/20 px-2.5 py-1 rounded-full uppercase tracking-wider hidden sm:inline-block">
                      ⇄ Drag &amp; Drop Enabled
                  ...
                  >
                    {variants[activeTab].images ? (
                        ...
                        {variants[activeTab].images
                          ...
                          .map((url, idx) => {
                            ...
                            const isBeingDragged =
                              draggedImage?.sourceType === "remastered" &&
                              draggedImage?.sourceIndex === idx &&
                              draggedImage?.variantIndex === activeTab;
                            const isOver = dragOverCardKey === cardKey;
                      ...
                      <p className="font-mono text-[10px] text-muted italic">
                        No custom remastered images uploaded. Drag images here
                        or upload below. (Inheriting from parent)
                      </p>
                    )}
                  </div>
                  ...
                  <div className="max-w-[400px]">
                    <CloudinaryUpload
                      onUpload={(urls) => {
                        setIsDirty(true);
                        const existing = variants[activeTab].images
                          ? variants[activeTab].images
                              .split(",")
                              .map((s) => s.trim())
                              .filter(Boolean)
                ...
                <div className="pt-6 border-t border-border/40 space-y-4">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="font-mono text-[10px] text-muted">
                        Variant-specific white background catalog images
                        (inherits parent if left blank). Drag to move across
                        sections.
                      </p>
                    </div>
                    <span className="font-mono text-[9px] text-accent/80 bg-accent/10 border border-accent/20 px-2.5 py-1 rounded-full uppercase tracking-wider hidden sm:inline-block">
                  ...
                  >
                    {variants[activeTab].whiteBackgroundImages ? (
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center gap-2 transition-all">
                                  ...
                                  <span className="font-mono text-[9px] text-accent uppercase tracking-widest bg-black/70 px-2 py-0.5 rounded border border-accent/30">
                                    Drag
                                  </span>
                                  {idx <
                                    variants[activeTab].whiteBackgroundImages
                      ...
                      <p className="font-mono text-[10px] text-muted italic">
                        No custom white-background images uploaded. Drag images
                        here or upload below. (Inheriting from parent)
                      </p>
                    )}
                  </div>
                  ...
                  <div className="max-w-[400px]">
                    <CloudinaryUpload
                      onUpload={(urls) => {
                        setIsDirty(true);
                        const existing = variants[activeTab]
                          .whiteBackgroundImages
                          ? variants[activeTab].whiteBackgroundImages
                              .split(",")
                              .map((s) => s.trim())
      ...
      {editingImageUrl && editingImageTarget && (
        <ProductImageEditor
          ...
          onSave={async (blob: Blob) => {
            await handleSaveEditedImageInForm(blob);
            setEditingImageUrl(null);
            setEditingImageTarget(null);
          }}
...
function CollapsibleCard({
  ...
  return (
    <section
      id={id}
      className="premium-card flex flex-col overflow-hidden mb-6"
    >
      <div
        onClick={onToggle}
      ...
      >
        <div className="flex items-center gap-4">
          {done ? (
            <span className="w-6 h-6 rounded-full bg-emerald-500/10 border border-emerald-500 text-emerald-400 flex items-center justify-center text-[10px] font-semibold">
              ✓
            </span>
          ) : warn ? (
            <span className="w-6 h-6 rounded-full bg-amber-500/10 border border-amber-500 text-amber-400 flex items-center justify-center text-[10px] font-semibold">
              !
            </span>
          ) : (
            <span className="w-6 h-6 rounded-full border border-border-strong text-muted flex items-center justify-center text-[10px] font-mono">
              {number}
            </span>
          )}
          <div>
            <h3 className="font-serif text-[18px] text-primary font-medium tracking-wide">
              {title}
            </h3>
        ...
        <span
          className={`text-[18px] text-muted transition-transform duration-200 ${isOpen ? "rotate-90" : ""}`}
        >
          ›
        </span>
      </div>
      {isOpen && <div className="p-6 space-y-6 bg-surface/30">{children}</div>}
    </section>
  );
}
