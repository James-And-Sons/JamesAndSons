...
function logInboundSale(
  ...
  try {
    ...
    if (err.code === "EROFS") {
      console.log(
        `[Inbound Webhook] Running in read-only serverless environment. File logging to ${err.path} skipped.`,
      );
    } else {
  ...
  try {
    ...
    if (!sku || typeof quantity !== "number" || quantity <= 0) {
      return NextResponse.json(
        { error: "Invalid sku or quantity in payload" },
        { status: 400 },
      );
    }
    ...
    console.log(
      `[Inbound Webhook] Processing sale from ${channelName} for SKU ${sku} (Qty: ${quantity})...`,
    );
    ...
    if (variant) {
      ...
      const newProdStock = Math.max(
        0,
        variant.product.stockQuantity - quantity,
      );
      ...
      console.log(
        `[Inbound Webhook] Variant SKU ${targetSku} stock updated: ${variant.stockQuantity} -> ${newVarStock}`,
      );
      ...
      if (!product) {
        console.warn(
          `[Inbound Webhook] SKU ${targetSku} not found in database.`,
        );
        logInboundSale(
          targetSku,
          quantity,
      ...
      updatedProductStock = newProdStock;
      console.log(
        `[Inbound Webhook] Product SKU ${targetSku} stock updated: ${product.stockQuantity} -> ${newProdStock}`,
      );
      logInboundSale(targetSku, quantity, channelName, "SUCCESS");
    }

    sendNotificationToAllAdmins({
      title: `📦 Marketplace Sale (${channelName})`,
      body: `Stock decremented for SKU ${targetSku} (Qty: ${quantity}). New stock: ${updatedProductStock}`,
      url: `/products`,
      type: "ORDER",
    }).catch(() => {});
