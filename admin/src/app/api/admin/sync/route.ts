...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === "list") {
      console.log(
        "[Manual Sync] Listing all product IDs for client-side batch execution...",
      );
    ...
    if (productId) {
      console.log(
        `[Manual Sync] Triggering sync for single product ID: ${productId}`,
      );
      ...
      console.log(
        `[Manual Sync] Found ${products.length} products. Syncing...`,
      );
      ...
      for (const product of products) {
        try {
          ...
          console.error(
            `[Manual Sync] Failed to sync SKU ${product.sku}:`,
            syncErr,
          );
          resultsSummary.push({
            sku: product.sku,
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (target === "amazon" || target === "both") {
      try {
        console.log(
          `[Manual Delete] Deleting SKU ${product.sku} and variants from Amazon...`,
        );
        ...
        if (product.variants && product.variants.length > 0) {
          for (const v of product.variants) {
            try {
              ...
              logs.push(
                `Successfully deleted variant SKU ${v.sku} from Amazon.`,
              );
            } catch (err: any) {
              console.error(
                `[Manual Delete] Failed to delete variant SKU ${v.sku} from Amazon:`,
                err,
              );
              logs.push(
                `Failed to delete variant SKU ${v.sku} from Amazon: ${err.message}`,
              );
            }
          }
        }
        ...
        try {
          ...
          logs.push(
            `Successfully deleted parent SKU ${product.sku} from Amazon.`,
          );
        } catch (err: any) {
          console.error(
            `[Manual Delete] Failed to delete parent SKU ${product.sku} from Amazon:`,
            err,
          );
          logs.push(
            `Failed to delete parent SKU ${product.sku} from Amazon: ${err.message}`,
          );
        }
      } catch (err: any) {
    ...
    if (target === "platform" || target === "both") {
      try {
        ...
        logs.push(
          `Successfully deleted product ${product.sku} from website platform database.`,
        );
      } catch (err: any) {
        ...
        return NextResponse.json(
          {
            error:
              "Failed to delete product from database due to an internal error",
            code: "DB_DELETION_FAILED",
            logs,
          },
          { status: 500 },
        );
    ...
    console.error("[Manual Delete] Error executing deletion:", message);
    return NextResponse.json(
      {
        error: "Internal deletion error processing request",
