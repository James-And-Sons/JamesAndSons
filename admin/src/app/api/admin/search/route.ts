...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const rfqs =
      searchPromises[2].status === "fulfilled" ? searchPromises[2].value : [];
    const customers =
      searchPromises[3].status === "fulfilled" ? searchPromises[3].value : [];

    if (searchPromises.some((res) => res.status === "rejected")) {
      console.warn(
        "[Admin Search Partial Rejection]:",
        searchPromises.filter((res) => res.status === "rejected"),
      );
    }
    ...
    console.error("[Admin Search Global Failure]:", error);
    return NextResponse.json(
      { error: "Search failed", code: "SEARCH_QUERY_ERROR" },
      { status: 500 },
    );
  }
}
