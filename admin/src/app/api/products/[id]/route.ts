  ...
  try {
    ...
    if (e.code === "P2003" || e.message?.includes("Foreign key constraint")) {
      return NextResponse.json(
        {
          error:
            "This product cannot be deleted because it is associated with existing Orders or RFQs. You can set its stock to 0 or archive it instead.",
        },
        { status: 400 },
      );
    }
...
export async function PATCH(
  ...
  try {
    const body = await req.json();
    const { images, whiteBackgroundImages } = body;
    const updated = await prisma.product.update({
      where: { id: params.id },
      data: {
        ...(images !== undefined && { images }),
        ...(whiteBackgroundImages !== undefined && { whiteBackgroundImages }),
      },
    });
