...
export default function LogisticsDashboardClient({
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "returns" | "warehouses" | "ndr"
  >("overview");
  const [isPending, startTransition] = useTransition();
  const [isSyncingStatuses, setIsSyncingStatuses] = useState(false);

  useEffect(() => {
    // Passive background sync on page mount
    fetch("/api/orders/sync-statuses").catch(() => {});
  }, []);
  ...
  // NDR form state
  const [selectedNDROrder, setSelectedNDROrder] = useState<OrderItem | null>(
    null,
  );
  ...
  const handleApproveReturn = (requestId: string) => {
    startTransition(async () => {
      ...
      if (res.success) {
        alert(
          "Return request approved and Shiprocket shipment created successfully!",
        );
      } else {
  ...
  const handleAddLocation = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        alert(
          "Pickup warehouse location registered successfully on Shiprocket!",
        );
  ...
  const handleScheduleReattempt = () => {
    ...
    startTransition(async () => {
      const res = await submitNDRAction(
        selectedNDROrder.trackingNumber || selectedNDROrder.awbNumber,
        ndrDate,
        ndrRemarks || "Admin rescheduled via NDR panel",
      );
      if (res.success) {
        ...
        alert(
          `Action failed: ${res.message || "Check if shipment is eligible for NDR reattempt"}`,
        );
      }
    });
  };
  ...
  const ndrOrders = orders.filter(
    (o) =>
      o.fulfillmentError?.includes("AWB") ||
      o.status === "PROCESSING" ||
      o.status === "SHIPPED",
  );
      ...
      {activeTab === "overview" && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="premium-card p-6 rounded-lg bg-gradient-to-br from-surface to-surface/90 border border-border flex flex-col justify-between">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-[12px] ${connectionStatus === "CONNECTED" ? "text-emerald-500" : "text-rose-500"}`}
                >
                  ● {connectionStatus}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between mt-3">
                <p className="font-mono text-[9px] text-muted m-0">
                  Top-ups in Shiprocket Console.
                </p>
                <button
                  onClick={handleSyncStatuses}
                  disabled={isSyncingStatuses}
                  className="px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/20 transition-all font-mono text-[9px] uppercase tracking-wider rounded-xs flex items-center gap-1.5 cursor-pointer"
                >
                  {isSyncingStatuses ? "Syncing..." : "Sync Statuses"}
                </button>
              </div>
            </div>
            ...
            <div className="premium-card p-6 rounded-lg flex flex-col justify-between">
              ...
              <p className="font-mono text-[9px] text-muted mt-3 m-0">
                Out of {orders.length} total active orders
              </p>
            </div>
            ...
            <div className="premium-card p-6 rounded-lg flex flex-col justify-between">
              ...
              <p className="font-mono text-[9px] text-muted mt-3 m-0">
                {returns.length} registered requests total
              </p>
            </div>
          </div>
          ...
          <div className="premium-card p-6 rounded-lg space-y-4">
            <h3 className="font-serif text-[18px] text-primary font-normal m-0">
              ...
              <div>
                ...
                <div className="w-full bg-border h-1.5 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${orders.length ? (returns.length / orders.length) * 100 : 0}%`,
                    }}
                  />
                </div>
              </div>
            ...
            <tbody className="divide-y divide-border/40 font-mono text-[12px]">
              {returns.map((r) => (
                <>
                  <tr
                    key={r.id}
                    className="hover:bg-surface-muted/20 cursor-pointer"
                    onClick={() =>
                      setExpandedReturnId(
                        expandedReturnId === r.id ? null : r.id,
                      )
                    }
                  >
                  ...
                  {expandedReturnId === r.id && (
                    <tr>
                      <td
                        colSpan={5}
                        className="px-8 py-4 bg-surface-muted/30 border-t border-b border-border/60"
                      >
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 font-mono text-[11px] text-primary">
                          <div>
                            <p className="font-bold text-accent uppercase tracking-wider mb-2">
                              Request Details
                            </p>
                            <p>
                            ...
                            <p>
                              <strong>Reason Note:</strong> {r.reason}
                            </p>
                            {r.adminNote && (
                            ...
                            {r.fulfillmentError && (
                              <div className="bg-rose-950/20 border border-rose-500/30 text-rose-400 p-3 rounded mt-2">
                                <strong>Logistics Warning:</strong>{" "}
                                {r.fulfillmentError}
                              </div>
                            )}
                          </div>
                          <div>
                            ...
                            {r.shipmentId && (
                              <p className="mt-3">
                                <strong>Shiprocket Shipment ID:</strong>{" "}
                                {r.shipmentId} <br />
                                <strong>AWB Number:</strong>{" "}
                                {r.awbNumber || "Assigning..."}
                              </p>
                            )}
                          </div>
                        </div>
                  ...
                  {rejectingId === r.id && (
                    <tr>
                      <td
                        colSpan={5}
                        className="px-6 py-4 bg-rose-950/10 border-t border-border"
                      >
                        <div className="flex flex-col gap-3 font-mono">
                          <label className="text-rose-400 text-[11px] uppercase tracking-wider">
        ...
        <div className="space-y-6">
          ...
          {showAddLocation && (
            <form
              ...
              className="premium-card p-6 rounded-lg space-y-4 font-mono text-[12px]"
            >
              <h3 className="font-serif text-[15px] text-primary font-normal uppercase tracking-wider m-0">
                Register New Pickup Location
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] text-muted uppercase tracking-wider block mb-1">
                    Pickup Location Name ID
                  </label>
                  <input
                    required
                    placeholder="e.g. Aligarh Primary Warehouse"
                    value={locationName}
                ...
                <div>
                  ...
                  <input
                    required
                    type="email"
                    placeholder="e.g. operations@jamesandsons.in"
                    value={email}
                ...
                <div className="md:col-span-2">
                  ...
                  <input
                    required
                    placeholder="e.g. Building B-24, Phase 2, Industrial Area"
                    value={address}
              ...
              <button
                disabled={isPending}
                type="submit"
                className="bg-accent text-black font-bold uppercase px-6 py-2.5 rounded-sm hover:bg-accent-hover transition-all"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {pickupLocations.map((loc: any, idx: number) => (
              <div
                key={idx}
                className="premium-card p-6 rounded-lg space-y-3 font-mono text-[11px] relative"
              >
                <div className="flex justify-between items-center border-b border-border pb-2 mb-2">
                  <span className="font-bold text-accent text-[12px] uppercase">
                    {loc.pickup_location}
                  </span>
                  {loc.status === "active" && (
                    <span className="bg-emerald-950 text-emerald-400 border border-emerald-500/20 px-2 py-0.5 rounded text-[9px] uppercase tracking-wider">
                      Active
                    </span>
                  )}
                </div>
                <p>
                  <strong>Contact:</strong> {loc.name}
                </p>
                <p>
                ...
                <p>
                  <strong>Address:</strong> {loc.address}, {loc.city},{" "}
                  {loc.state} - {loc.pin_code}
                </p>
              </div>
            ))}
            ...
            {pickupLocations.length === 0 && (
              <div className="col-span-2 premium-card p-8 text-center text-muted font-mono text-[10px] uppercase tracking-widest">
                No active pickup locations retrieved. Check your Shiprocket
                credentials.
              </div>
            )}
          </div>
      ...
      {activeTab === "ndr" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-border/40 font-mono text-[12px]">
                {ndrOrders.map((o) => (
                  <tr
                    key={o.id}
                    className={`hover:bg-surface-muted/20 ${selectedNDROrder?.id === o.id ? "bg-surface-muted/40 border-l-2 border-accent" : ""}`}
                  >
                    <td className="px-6 py-4 font-bold text-accent">
                      {o.orderNumber}
                    </td>
                    ...
                    <td className="px-6 py-4">
                      {o.trackingNumber || o.awbNumber || "No AWB assigned"}
                    </td>
                    <td className="px-6 py-4">
                      <span className="status-pill status-processing">
                        <span>{o.status}</span>
          ...
          <div className="premium-card p-6 rounded-lg space-y-4 font-mono text-[12px]">
            <h3 className="font-serif text-[16px] text-primary font-normal uppercase tracking-wider m-0">
              Logistics Re-attempt
            </h3>
            {selectedNDROrder ? (
              <div className="space-y-4">
                <div className="bg-surface-muted p-4 rounded border border-border">
                  <p>
                    ...
                    {selectedNDROrder.orderNumber}
                  </p>
                  <p>
                    <strong>Customer:</strong> {selectedNDROrder.customerName}
                  </p>
                  <p>
                    <strong>AWB Code:</strong>{" "}
                    {selectedNDROrder.trackingNumber ||
                      selectedNDROrder.awbNumber}
                  </p>
                </div>
              ...
              <div className="text-center text-muted py-8 font-mono text-[10px] uppercase tracking-wider italic">
                Select an active shipment from the queue to schedule a delivery
                re-attempt.
              </div>
            )}
          </div>
