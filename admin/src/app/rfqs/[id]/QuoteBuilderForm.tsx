...
import { ChevronRight, ChevronLeft, FileText, Cloud, Check, X } from 'lucide-react';
...
export default function QuoteBuilderForm({ rfq }: { rfq: any }) {
  ...
  // ── Mobile Wizard State ───────────────────────────────────────────────────
  const MOBILE_STEPS = ['Item Pricing', 'Adjustments', 'Summary', 'Actions'] as const;
    ...
    <div className="block md:hidden flex flex-col min-h-0">
      ...
      <div className="sticky top-[64px] z-20 bg-background border-b border-border px-4 py-3">
          ...
          {MOBILE_STEPS.map((label, idx) => (
            <button
              ...
              className={`flex-1 flex flex-col items-center gap-1 p-1 cursor-pointer transition-colors ${
                idx === mobileStep ? 'text-accent' : idx < mobileStep ? 'text-emerald-500' : 'text-muted'
              }`}
              aria-current={idx === mobileStep ? 'step' : undefined}
            >
              <span className={`w-6 h-6 rounded-full border-2 flex items-center justify-center font-mono text-[10px] ${
                idx < mobileStep
              ...
              <span className="font-mono text-[8px] uppercase tracking-wider text-center leading-tight hidden xs:block">{label}</span>
            </button>
          ))}
        </div>
        {/* Progress line */}
        <div className="mt-2 h-0.5 bg-border rounded-full overflow-hidden">
          <div
            ...
            style={{ width: `${((mobileStep) / (MOBILE_STEPS.length - 1)) * 100}%` }}
          />
        </div>
      </div>
      ...
      {mobileStep === 0 && (
        <div className="flex-1 p-4 space-y-4">
          <h2 className="font-serif text-[20px] text-primary font-light">Review & Price Items</h2>
              ...
              <div className="p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3 font-mono text-center">
                  <div className="bg-surface-muted/50 p-2 rounded-sm border border-border/30">
                    <span className="text-[9px] text-muted uppercase tracking-wider block">Qty</span>
                    <span className="text-[16px] text-primary font-semibold">{item.quantity}</span>
                  </div>
                  <div className="bg-surface-muted/50 p-2 rounded-sm border border-border/30">
                    <span className="text-[9px] text-muted uppercase tracking-wider block">Standard B2B</span>
                    <span className="text-[13px] text-muted line-through">{formatPriceInline(item.product.b2bPrice)}</span>
                  </div>
                </div>
                <div>
                  <label className="font-mono text-[9px] uppercase tracking-[0.12em] text-accent block mb-1.5">Approved Unit Price (₹)</label>
                  ...
                  <div className="font-mono text-[10px] text-muted mt-1">
                    Line total: <span className="text-accent font-semibold">{formatPriceInline(prices[item.id] * item.quantity)}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      {mobileStep === 1 && (
        <div className="flex-1 p-4 space-y-4">
          <h2 className="font-serif text-[20px] text-primary font-light">Quote Adjustments</h2>
          <div className="bg-surface border border-border rounded-lg p-4 space-y-4">
            <div>
              <label className="font-mono text-[9px] uppercase tracking-[0.12em] text-muted block mb-1.5">Discount (₹)</label>
              <input
            ...
            <div>
              <label className="font-mono text-[9px] uppercase tracking-[0.12em] text-muted block mb-1.5">Freight / Shipping (₹)</label>
              <input
                type="number" inputMode="numeric"
                disabled={isLocked || isPending}
            ...
            <label className="flex items-center gap-3 p-4 bg-surface-muted/40 border border-border rounded-sm cursor-pointer min-h-[52px]">
              ...
              />
              <div>
                <div className="font-mono text-[11px] text-primary">Include 18% GST</div>
                <div className="font-mono text-[10px] text-muted">{formatPriceInline(taxAmount)} will be added</div>
              </div>
            </label>
          </div>
        </div>
      ...
      {mobileStep === 2 && (
        <div className="flex-1 p-4 space-y-4">
          <h2 className="font-serif text-[20px] text-primary font-light">Quote Summary</h2>
          <div className="bg-surface border border-border rounded-lg overflow-hidden">
            <div className="divide-y divide-border/40">
              <div className="flex justify-between px-4 py-3.5">
                ...
                <span className="font-mono text-[13px] text-primary tabular-nums">{formatPriceInline(subtotal)}</span>
              </div>
              {discountAmount > 0 && (
                <div className="flex justify-between px-4 py-3.5">
                  <span className="font-mono text-[11px] text-amber-500">Discount</span>
                  <span className="font-mono text-[13px] text-amber-500 tabular-nums">-{formatPriceInline(discountAmount)}</span>
                </div>
              )}
              {includeGst && (
                <div className="flex justify-between px-4 py-3.5">
                  <span className="font-mono text-[11px] text-muted">GST (18%)</span>
                  <span className="font-mono text-[13px] text-primary tabular-nums">{formatPriceInline(taxAmount)}</span>
                </div>
              )}
              {shippingAmount > 0 && (
                <div className="flex justify-between px-4 py-3.5">
                  <span className="font-mono text-[11px] text-muted">Freight</span>
                  <span className="font-mono text-[13px] text-primary tabular-nums">{formatPriceInline(shippingAmount)}</span>
                </div>
              )}
              <div className="flex justify-between px-4 py-4 bg-accent/5">
                <span className="font-mono text-[11px] uppercase tracking-wider text-accent font-semibold">Final Total</span>
                <span className="font-serif text-[24px] text-accent">{formatPriceInline(totalAmount)}</span>
              </div>
            </div>
          </div>
          {/* Items recap */}
          <div className="space-y-2">
            <p className="font-mono text-[10px] uppercase tracking-wider text-muted">Items in this quote</p>
            {rfq.items.map((item: RFQItem) => (
              <div key={item.id} className="flex items-center justify-between py-2 border-b border-border/30">
                <div>
                  <div className="font-mono text-[11px] text-primary">{item.product.name}</div>
                  <div className="font-mono text-[9px] text-muted">× {item.quantity} units @ {formatPriceInline(prices[item.id])}</div>
                </div>
                <span className="font-mono text-[12px] text-accent tabular-nums">{formatPriceInline(prices[item.id] * item.quantity)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {mobileStep === 3 && (
        <div className="flex-1 p-4 space-y-4">
          <h2 className="font-serif text-[20px] text-primary font-light">Finalize Quote</h2>
          {/* Tools */}
          <div className="bg-surface border border-border rounded-lg divide-y divide-border/40">
            ...
            >
              ...
              <div>
                <div className="font-mono text-[11px] uppercase tracking-wider">Generate PDF Quote</div>
                <div className="font-mono text-[9px] text-muted">Print or share with client</div>
              </div>
            ...
            >
              ...
              <div>
                <div className="font-mono text-[11px] uppercase tracking-wider">{zohoSyncing ? 'Syncing...' : 'Sync to Zoho CRM'}</div>
                <div className="font-mono text-[9px] text-muted">Push lead to your CRM</div>
              </div>
              {zohoSyncing && <span className="ml-auto inline-block animate-spin border border-t-transparent border-accent rounded-full w-4 h-4" />}
            </button>
          </div>
          ...
          {!isLocked ? (
            ...
            <div className="p-4 bg-surface-muted/40 border border-border rounded-lg text-center">
              <p className="font-mono text-[11px] uppercase tracking-[0.15em] text-muted">
                This quotation has been {rfq.status.toLowerCase()} and cannot be modified.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="sticky-action-bar bg-surface/95 backdrop-blur-md border-t border-border flex gap-3 px-4 py-3">
        ...
        {mobileStep < MOBILE_STEPS.length - 1 ? (
          <button
            type="button"
            onClick={() => setMobileStep(s => Math.min(s + 1, MOBILE_STEPS.length - 1))}
