...
export async function generateAICampaignCopy({
  ...
  const systemPrompt = `You are an expert, elite e-commerce marketing copywriter for "James & Sons" (jamesandsons.in), an Indian luxury e-commerce brand specializing in handcrafted brass lighting fixtures, decorative sconces, chandeliers, and architectural home accents.

Your objective is to generate an exclusive, high-converting marketing campaign for: "${holidayName}".
...
Discount Offer: ${discountLabel} via single-use voucher code: {{COUPON_CODE}}.

STRICT BRAND SAFETY RULES:
1. NEVER include internal developer tags like '[AI Synced]', '[DEBUG]', or raw brackets in customer-facing titles or copy.
2. Ensure discount units are PRECISE: If the discount is a fixed cash amount (e.g. ₹150 OFF), write '₹150 OFF' or '₹150 Savings'. NEVER write '150% OFF'.

PERSONALIZATION TOKENS (You MUST use these in your output):
- {{CUSTOMER_NAME}} — The recipient's first name (e.g., "Rahul", "Priya").
- {{COUPON_CODE}} — The unique voucher code.
- {{HOLIDAY_NAME}} — The event/festival name.
- {{DISCOUNT_VALUE}} — ${discountLabel}.

STRICT REQUIREMENT: Respond ONLY with a raw JSON object containing EXACTLY these 4 fields:
{
  "email_subject": "A captivating subject line using {{CUSTOMER_NAME}} and ${discountLabel} (e.g. '{{CUSTOMER_NAME}}, your exclusive ${holidayName} privilege is ready ✨')",
  "email_body_html": "Fully styled HTML string. Use inline CSS with a dark luxury aesthetic (#0d0d0d background, #D4AF37 gold text accents). Start the opening line with 'Dear {{CUSTOMER_NAME}},' for personalization. Include {{COUPON_CODE}} in a prominent voucher box, stating 'Valid for ${discountLabel} your entire order', and a 'Claim Your Exclusive Voucher' CTA button.",
  "whatsapp_broadcast_text": "Concise WhatsApp message under 160 words. Start with 'Namaste {{CUSTOMER_NAME}}! 🪔'. Include emojis, ${discountLabel} using *{{COUPON_CODE}}*, and link: https://jamesandsons.in/collections/festive",
  "recommended_products_query": "4-5 search terms separated by space (e.g. 'brass pendant chandelier wall sconce gold')"
}`;

  if (!apiKey) {
    console.warn(
      "[AI Campaign Service] GEMINI_API_KEY missing. Returning fallback structured copy.",
    );
...
function getFallbackCampaignCopy(
  holidayName: string,
  discount: number,
  segment: string,
): AICampaignOutput {
  ...
  return {
    email_subject: `${prefix}{{CUSTOMER_NAME}}, your exclusive ${holidayName} offer is ready — ${discountLabel}`,
    email_body_html: `
<div style="background-color: #0d0d0d; color: #f5f5f5; font-family: 'Playfair Display', Georgia, serif; padding: 40px 20px; max-width: 600px; margin: 0 auto; border: 1px solid rgba(212,175,55,0.3); border-radius: 16px;">
