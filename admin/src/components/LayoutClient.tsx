...
function HeaderTitle() {
  ...
  const getBreadcrumbs = () => {
    ...
    if (pathname === "/campaigns")
      return [{ label: "Marketing", href: "/campaigns" }];
    if (pathname === "/promotions")
      return [{ label: "Coupons & Promotions", href: "/promotions" }];
    if (pathname === "/affiliates")
      return [{ label: "Affiliates", href: "/affiliates" }];
    if (pathname === "/tickets")
      return [{ label: "Support Tickets", href: "/tickets" }];
    if (pathname === "/customers")
      return [{ label: "Customers", href: "/customers" }];
    if (pathname === "/account")
      return [{ label: "Settings", href: "/account" }];
      ...
      {breadcrumbs.map((crumb, idx) => {
        ...
        return (
          <div key={idx} className="flex items-center gap-1.5">
            ...
            {isLast || !crumb.href ? (
              <span className="text-primary/95 font-semibold">
                {crumb.label}
              </span>
            ) : (
              <Link
    ...
    <TenantProvider config={DEFAULT_TENANT_CONFIG}>
      <ThemeProvider>
        <SidebarProvider>
          ...
          <div
            className={`flex-1 flex flex-col min-h-screen transition-all duration-300 min-w-0 overflow-x-hidden ${isLoginPage ? "ml-0" : "lg:ml-[260px] ml-0"}`}
          >
            {!isLoginPage && (
              <header
                className={`pt-[env(safe-area-inset-top,0px)] h-[calc(64px+env(safe-area-inset-top,0px))] bg-background/90 backdrop-blur-md border-b border-border flex items-center justify-between px-4 lg:px-10 sticky top-0 z-40 transition-colors duration-300 ${
              ...
              <div
                className={
                  isLoginPage || isChatPage
                    ...
                    : "max-w-[1200px] mx-auto w-full min-w-0 overflow-x-hidden"
                }
              >
                {children}
              </div>
