  ...
  const categoryImages = useMemo(() => {
    ...
    displayCategories.forEach((cat) => {
      ...
      const matchingProducts = products.filter((p) => {
        ...
        return (
          pCol.includes(catSlug) ||
          catSlug.includes(pCol) ||
          pCol.includes(catName)
        );
      });

      // Extract all product images for this category
      const pool = matchingProducts
        .flatMap((p) => [
          ...(p.images || []),
  ...
  return (
    <section className="section" id="categories">
      <div className="section-header">
        ...
        <Link href="/collections" className="link-all nav-haptic">
      ...
      <div className="space-grid hidden md:grid">
        ...
        <Link
          ...
          className="space-card block overflow-hidden group"
        >
          {mainCatImage ? (
            <Image
              src={mainCatImage}
          ...
          {!mainCatImage && (
            ...
            >
              <i
                className={`ti ${categoryIcons[mainCat.slug] || "ti-lamp"}`}
                style={{
                  fontSize: "80px",
                  color: "var(--gold)",
                  opacity: 0.25,
          ...
          <div className="space-card-arrow" style={{ zIndex: 3 }}>
            ↗
          </div>
          <div className="space-card-content" style={{ zIndex: 3 }}>
            ...
            <div className="space-card-count">
              {mainCat._count?.products || 0} Designs
            </div>
          </div>
        </Link>
        ...
        <div className="space-grid-right">
            ...
            return (
              ...
              >
                ...
                {!catImage && (
                  ...
                  >
                    <i
                      className={`ti ${categoryIcons[cat.slug] || "ti-lamp"}`}
                      style={{
                        fontSize: "42px",
                        color: "var(--gold)",
                        opacity: 0.25,
          ...
          {Array.from({ length: Math.max(0, 4 - otherCats.length) }).map(
            (_, i) => (
              <div
                key={`empty-${i}`}
          ...
          return (
            ...
            >
              {catImage ? (
                ...
                <i
                  className={`ti ${categoryIcons[cat.slug] || "ti-lamp"} mobile-space-icon`}
                  aria-hidden="true"
                />
              )}
