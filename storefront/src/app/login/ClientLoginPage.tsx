...
export default function ClientLoginPage({ searchParams, referer }: Props) {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'stretch', paddingTop: '64px' }}>
      ...
      <div style={{ flex: 1, flexDirection: 'column', justifyContent: 'center', padding: '60px 48px', position: 'relative', background: 'var(--obsidian)' }} className="hidden md:flex">
      ...
      {/* ── Mobile Form ── */}
      <div className="md:hidden" style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--obsidian)', paddingBottom: '40px' }}>
        <div className="mobile-section-intro">
          <div>
            <div className="section-label" style={{ marginBottom: '2px' }}>{isLogin ? 'Returning Client' : 'New Account'}</div>
        ...
        <div style={{ padding: '0 24px' }}>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: 'var(--text-muted)', marginBottom: '32px', lineHeight: 1.6 }}>
            {isLogin
              ? 'Sign in to access your orders, RFQs, and exclusive B2B pricing.'
              : 'Create your account to explore our full catalogue, request quotes, and track orders.'}
          </p>
          ...
          <form style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <button
              formAction={isLogin ? login : signup}
              className="btn-primary"
              style={{ marginTop: '8px', width: '100%', padding: '16px', borderRadius: '12px' }}
            >
              {isLogin ? 'Sign In' : 'Create Account'}
            </button>
            ...
            {/* Error message */}
            {searchParams?.message && (
              <div style={{ padding: '12px 16px', background: 'rgba(196,90,90,0.08)', border: '1px solid rgba(196,90,90,0.2)', fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--red)', borderRadius: '8px' }}>
                {searchParams.message}
              </div>
            )}
            ...
            <div style={{ marginTop: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '8px', textTransform: 'uppercase', color: 'var(--text-dim)', letterSpacing: '0.1em' }}>Or Continue With</span>
                <div style={{ flex: 1, height: '0.5px', background: 'var(--border)' }} />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <button 
                  type="button" 
                  onClick={() => signInWithSocial('google', nextUrl)}
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '14px', background: 'var(--surface)', border: '1px solid var(--border)', fontFamily: 'var(--font-mono)', fontSize: '9px', textTransform: 'uppercase', color: 'var(--cream)', borderRadius: '12px' }}
                >
                  <svg viewBox="0 0 24 24" width="14" height="14"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"></path><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"></path><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05"></path><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"></path></svg>
                  Google
                </button>
                <button 
                  type="button" 
