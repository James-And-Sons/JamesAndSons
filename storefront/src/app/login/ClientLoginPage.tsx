...
export default function ClientLoginPage({ searchParams, referer }: Props) {
  ...
  const [localSuccess, setLocalSuccess] = useState<string | null>(searchParams.message || null)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = (isLogin ? await login(formData) : await signup(formData)) as any
      if (res && res.error) {
        ...
        if (errLower.includes('already') || errLower.includes('registered') || errLower.includes('exists')) {
          setLocalError('already_exists')
        } else if (errLower.includes('invalid') || errLower.includes('credentials') || errLower.includes('not found') || errLower.includes('combination')) {
        ...
        <div style={{ maxWidth: '440px', width: '100%', margin: '0 auto' }}>
          ...
          {localError === 'already_exists' && (
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'var(--gold-light)' }}>
                <i className="ti ti-alert-triangle" style={{ fontSize: '18px' }} />
                <span style={{ fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Account Exists</span>
              </div>
          ...
          {localError && localError !== 'already_exists' && (
            <div style={{
              ...
              <i className="ti ti-alert-triangle" style={{ fontSize: '16px', color: '#f87171' }} />
          ...
          {localSuccess && (
            <div style={{
              ...
              <i className="ti ti-circle-check" style={{ fontSize: '16px', color: '#34d399' }} />
          ...
          <div style={{ marginBottom: '24px' }}>
            <button 
              ...
              disabled={loading}
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', width: '100%', padding: '16px', background: 'var(--surface)', border: '1px solid var(--border)', fontFamily: 'var(--font-mono)', fontSize: '10px', textTransform: 'uppercase', color: 'var(--cream)', borderRadius: '12px', cursor: 'pointer', letterSpacing: '0.15em', opacity: loading ? 0.6 : 1, minHeight: '44px' }}
            >
          ...
          <form onSubmit={handleSubmit}>
            <fieldset disabled={loading} style={{ border: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                {!isLogin && (
                  <div style={{ 
                    ...
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', textTransform: 'uppercase', color: 'var(--text-muted)', letterSpacing: '0.05em' }}>Password Strength:</div>
                    {[
                      { text: 'At least 8 characters', valid: isLengthValid },
                      { text: 'One uppercase letter (A-Z)', valid: hasUppercase },
                      { text: 'One number (0-9)', valid: hasNumber }
                    ].map((req, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px', color: req.valid ? 'var(--green)' : 'var(--text-dim)', transition: 'all 0.2s' }}>
                        <i className={`ti ${req.valid ? 'ti-circle-check-filled text-[#10b981]' : 'ti-circle text-white/20'}`} style={{ fontSize: '14px' }} />
                        <span style={{ textDecoration: req.valid ? 'line-through' : 'none', color: req.valid ? 'var(--text-muted)' : 'var(--text-dim)' }}>{req.text}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {isLogin && (
                  <div style={{ textAlign: 'right' }}>
                    <a href="/forgot-password" style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--text-muted)', textDecoration: 'none', textTransform: 'uppercase', minHeight: '44px', display: 'inline-flex', alignItems: 'center' }}>Forgot Password?</a>
                  </div>
                )}
              </div>
              ...
              {isLogin && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', margin: '4px 0' }}>
                  ...
                  />
                  <label htmlFor="rememberMe" style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--text-muted)', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    Remember Me
                  </label>
                </div>
              )}
              ...
              <button
                ...
                style={{ marginTop: '8px', width: '100%', padding: '16px', borderRadius: '12px', letterSpacing: '0.2em', cursor: 'pointer', minHeight: '44px' }}
              >
              ...
              <div className="md:hidden" style={{ textAlign: 'center', marginTop: '8px' }}>
                ...
                <button
                  ...
                  style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'var(--gold)', background: 'transparent', border: 'none', textTransform: 'uppercase', padding: 0, minHeight: '44px' }}
                >
                  {isLogin ? 'Create Account →' : 'Sign In →'}
                </button>
              </div>
...
function AuthInput({ 
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', position: 'relative', width: '100%' }}>
      <label htmlFor={id} style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: '0.15em', color: 'var(--text-muted)', textTransform: 'uppercase' }}>
        {label}
      </label>
      <div style={{ position: 'relative', width: '100%' }}>
        <input
          ...
          style={{
            ...
            borderBottom: error ? '1px solid #f87171' : '1px solid var(--border-gold)',
            color: 'var(--text)',
      ...
      {error && (
        <span id={`${id}-error`} style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: '#f87171', marginTop: '2px' }}>
          {error}
        </span>
      )}
    </div>
