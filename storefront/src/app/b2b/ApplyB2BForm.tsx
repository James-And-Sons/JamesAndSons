...
export default function ApplyB2BForm({ userEmail }: { userEmail: string }) {
  ...
  const gstValidation =
    gstin.trim().length >= 15 ? validateGstinFormat(gstin) : null;

  async function handleSubmit(formData: FormData) {
    if (gstin.trim()) {
      const check = validateGstinFormat(gstin);
      if (!check.isValid) {
        setError(`Invalid GSTIN: ${check.error}`);
        return;
      }
    }
  ...
  if (success) {
    return (
      <div
        style={{
          padding: "40px",
          background: "rgba(90,196,120,0.05)",
      ...
      >
        ...
        >
          Welcome to the Trade Programme
        </h3>
        <p
          style={{
        ...
        >
          Your B2B account has been activated. You can now access exclusive
          trade pricing and submit RFQs from your account dashboard.
        </p>
      </div>
    );
  }
        ...
        >
          <div>
            <FormInput
              ...
              placeholder="e.g. 09AABCJ8243A1ZX"
            />
            {gstin.length > 0 && (
              <div
                style={{
              ...
              >
                {gstValidation?.isValid ? (
                  ...
                  <span style={{ color: "#f87171" }}>
                    ✕{" "}
                    {gstValidation?.error || "Invalid 15-digit GSTIN structure"}
                  </span>
                )}
              </div>
            )}
