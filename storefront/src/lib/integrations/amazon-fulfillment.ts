...
export async function fulfillAmazonOrder(orderId: string): Promise<void> {
  console.log(
    `[Amazon Fulfillment] Starting fulfillment for JNS order ${orderId}...`,
  );

  // Run standard fulfillment (Shiprocket AWB + invoice email + Zoho sales order)
  const result = await fulfillPaidOrder({ orderId, forceFulfillment: true });

  if (!result.success) {
    console.error(
      `[Amazon Fulfillment] Standard fulfillment failed for order ${orderId}:`,
      (result as any).error,
    );
    return;
  }
  ...
  if ((result as any).alreadyProcessed) {
    console.log(
      `[Amazon Fulfillment] Order ${orderId} already processed. Skipping shipment confirm.`,
    );
    return;
  }
  ...
  if (!order || !order.amazonOrderId) {
    console.warn(
      `[Amazon Fulfillment] Order ${orderId} not found or has no amazonOrderId.`,
    );
    return;
  }
  ...
  if (!order.trackingNumber) {
    console.warn(
      `[Amazon Fulfillment] No tracking number on order ${orderId} — skipping Amazon shipment confirm.`,
    );
    await prisma.order.update({
      ...
      data: {
        fulfillmentError:
          ...
          " | Amazon shipment NOT confirmed: no tracking number.",
      },
    });
    return;
  }
...
export async function confirmAmazonShipment(
  ...
  console.log(
    `[Amazon Fulfillment] Uploaded shipment XML to Amazon feed document ${feedDocumentId}.`,
  );
