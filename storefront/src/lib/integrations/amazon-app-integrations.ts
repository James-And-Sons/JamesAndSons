...
export async function createAppNotification(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.warn(
        `[Amazon AppIntegrations] Failed to create notification (${res.status}): ${errorText}`,
      );
      return {
        success: false,
...
/**
 * High-level helper: Send operational alert to Seller Central notification widget
 */
export async function sendSellerCentralOperationalAlert({
  templateId,
  orderNumber,
  message,
}: {
