...
export async function getEligibleShippingServices(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      // Fallback fallback offer mock structure if seller central handles ATS direct assignment
      return {
        success: true,
        services: [
          {
...
export async function createAmazonMfnShipment(
  ...
  try {
    ...
    // Fallback registration if ATS handles slot via Seller Central confirmation feed
