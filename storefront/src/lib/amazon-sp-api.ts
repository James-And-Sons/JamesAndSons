...
export async function getLwaAccessToken(): Promise<string> {
  ...
  if (!clientId || !clientSecret || !refreshToken) {
    throw new Error(
      "[Amazon SP-API] Missing LWA credentials: AMAZON_LWA_CLIENT_ID, AMAZON_LWA_CLIENT_SECRET, AMAZON_LWA_REFRESH_TOKEN",
    );
  }
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `[Amazon SP-API] LWA token exchange failed: ${res.status} — ${text}`,
    );
  }
...
/**
 * Obtains a Restricted Data Token (RDT) for accessing PII (Personally Identifiable Information)
 * such as full customer shipping address and buyer name from Amazon SP-API.
 */
export async function getRestrictedDataToken(
  dataElements: string[] = ["buyerInfo", "shippingAddress"],
): Promise<string> {
  ...
  try {
    ...
    const res = await signedSpApiFetch(
      "/tokens/2021-03-01/restrictedDataTokens",
      lwaToken,
      config,
      {
        method: "POST",
    ...
    if (!res.ok) {
      const text = await res.text();
      console.warn(
        `[Amazon SP-API RDT Warning] RDT token request status ${res.status}: ${text}. Using standard LWA token.`,
      );
      return lwaToken;
    }
    ...
    if (data?.restrictedDataToken) {
      console.log(
        "[Amazon SP-API] RDT PII Access Token obtained successfully.",
      );
      return data.restrictedDataToken;
    }
  } catch (err) {
    console.warn("[Amazon SP-API RDT Exception]:", err);
  }
